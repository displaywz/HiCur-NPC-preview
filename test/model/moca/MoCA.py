...
from model.moca.moeblock import MixtralSparseMoeBlock
from transformers import MixtralConfig
from model.llaval3.model.builder import load_pretrained_model
from model.llaval3.utils import disable_torch_init
from model.llaval3.mm_utils import tokenizer_image_token, process_images, get_model_name_from_path
import os
...
class ConvNeXtUNet(nn.Module):
    def __init__(self,):
        super().__init__()
        self.pre_model = timm.create_model('convnextv2_base',pretrained=True,pretrained_cfg_overlay=dict(file="/data/wzp/wz/model/convnext_v2_base/pytorch_model.bin"))
...
class MoCACUnet(torch.nn.Module):
    def __init__(self):
        ...
        self.convnextunet = SegModel.load_from_checkpoint('/data/wzp/wz/LLaVA/seg_ckpt/last.ckpt').model
...
class MoCANet(torch.nn.Module):
    def __init__(self):
        ...
        self.moca = PModel.load_from_checkpoint('/data/wzp/wz/LLaVA/pre_ckpt/last-v1.ckpt').pmodel
    def forward(self,x):
        return self.moca(x)
    def get_pre(self,x):
        return self.moca(x)
